...
async def start(update: Update, context: CallbackContext):
    ...
    await update.message.reply_voice(voice=InputFile(voice_byte_arr, filename="captcha.mp3"),
                                     caption="Enter the numbers you have heard.")
...
async def check_captcha(update: Update, context: CallbackContext):
    ...
    if user_answer == correct_answer:
        ...
        keyboard = [
            [InlineKeyboardButton("Go to the channel", url=one_time_link)]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text("You have passed the captcha.", reply_markup=reply_markup)
    else:
        await update.message.reply_text("You didn't pass the captcha.")
